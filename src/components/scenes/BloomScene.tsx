...
function Content({ p, slots }: { p: MotionValue<number>; slots: PublicSlot[] }) {
  ...
  const memory = slots[0]?.memory ?? null;
  ...
  return (
    <SceneCamera progress={p} config={CAMERA_PRESETS.bloom}>
      ...
      <div className="absolute left-1/2 top-[30%] w-52 -translate-x-1/2 sm:w-64">
        <motion.div style={{ opacity: photoOpacity, scale: photoScale }}>
          <motion.div style={{ y: fallY, x: fallX, rotate: fallRotate }}>
            ...
            {memory ? (
              <Polaroid
                src={memory.mediaUrl}
                alt={memory.title || "the first memory"}
                aspectRatio="3/4"
                caption={memory.caption || "where it started"}
              />
            ) : (
...
export default function BloomScene({ slots }: { slots: PublicSlot[] }) {
  return (
    <SceneShell vh={340}>
