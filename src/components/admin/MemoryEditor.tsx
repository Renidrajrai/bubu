...
export default function MemoryEditor({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-chocolate/30 p-4">
      <form
        onSubmit={handleSave}
        onChange={markDirty}
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <h2 className="font-display text-sm font-medium">edit memory</h2>
          <span className="text-[10px]">
            {saving === "saving" && <span className="text-text-secondary">saving…</span>}
            {saving === "saved" && <span className="text-cocoa">saved ✓</span>}
            {saving === "error" && <span className="text-rose">error</span>}
          </span>
        </div>
